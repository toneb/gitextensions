...
namespace GitUI
{
    ...
    /// Base class for a <see cref="UserControl"/> requiring <see cref="GitModule"/> and <see cref="GitUICommands"/>.
    /// </summary>
    public class GitModuleControlAvalonia : GitExtensionsControlAvalonia, IGitModuleControl
    {
        private readonly object _lock = new();
        private int _isDisposed;
        private IGitUICommandsSource? _uiCommandsSource;
        ...
        protected GitModuleControlAvalonia()
        {
        }
        ...
        protected override CommandStatus ExecuteCommand(int command)
        {
            ...
            CommandStatus ExecuteScriptCommand()
            {
                ...
                if (revisionGridControl is null)
                {
                    revisionGridControl = (VisualRoot as GitModuleFormAvalonia)?.RevisionGridControl;
                }

                IScriptsRunner scriptsRunner = UICommands.GetRequiredService<IScriptsRunner>();

                return scriptsRunner.RunScript(command, VisualRoot as GitModuleFormAvalonia, revisionGridControl);
#else
