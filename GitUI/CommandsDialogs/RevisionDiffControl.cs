...
namespace GitUI.CommandsDialogs
{
    public partial class RevisionDiffControl : GitModuleControl
    {
        ...
        {
            List<FileStatusItem> selectedItems = DiffFiles.SelectedItems.ToList();
        ...
        {
            (FindForm() as FormBrowseWrapper)?.SetPathFilter(string.Join(" ", DiffFiles.SelectedItems.Select(f => f.Item.Name.ToPosixPath().QuoteNE())));
        }

        private void UpdateStatusOfMenuItems()
        {
            ContextMenuSelectionInfo selectionInfo = GetSelectionInfo();
        ...
        private void OpenInFileTreeTab(bool requestBlame)
        {
            ...
            int? line = DiffText.Visible ? DiffText.CurrentFileLine : BlameControl.CurrentFileLine;
            (FindForm() as FormBrowseWrapper)?.ExecuteCommand(FormBrowse.Command.FocusFileTree);
        ...
        private void diffUpdateSubmoduleMenuItem_Click(object sender, EventArgs e)
        {
            ...
            FormProcess.ShowDialog(FindForm() as FormBrowseWrapper, UICommands, arguments: Commands.SubmoduleUpdate(submodules), Module.WorkingDir, input: null, useDialogSettings: true);
            RequestRefresh();
        }
